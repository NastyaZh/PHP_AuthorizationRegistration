!(function($) {
            ...
            if (data.status === 'ok') {
                ...
                if (data.code === 'main') {
                    if ($mainErrorContainer !== null) {
                        $mainErrorContainer.html('<p class="error active">' + data.message + '</p>');
                    } else {
                        $form.append('<p class="error active">' + data.message + '</p>');
                    }
                } else {
                    var $errField = $form.find('[name|="' + data.code + '"]');
                    var $fieldErrorContainer = $form.find(`.${data.code}Error`);
                    $fieldErrorContainer.html('<p class="error active">' + data.message + '</p>');
                    $errField.focus();
                }
            }
    ...
    function tr(params, lang) {
            ...
            messages.en_EN = {
                'error': {
                    'form-submit': 'An error occurred while submitting the form',
                    'ajax-request': 'An error occurred while sending the request'
                },
